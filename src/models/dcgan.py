    ...
    def __init__(self, img_size, d_conv_dim, d_spectral_norm, attention, attention_after_nth_dis_block, activation_fn, conditional_strategy,
        ...
        if d_spectral_norm:
            if self.conditional_strategy == 'ECGAN':
                self.linear1 = snlinear(in_features=512, out_features=num_classes)
            ...
            if self.conditional_strategy == 'ECGAN':
                self.linear1 = linear(in_features=512, out_features=num_classes)
    ...
    def forward(self, x, label, evaluation=False):
        with torch.cuda.amp.autocast() if self.mixed_precision is True and evaluation is False else dummy_context_mgr() as mp:
            ...
            if self.conditional_strategy == 'no':
                ...
                cls_output = self.linear1(h)
                cond_output = (cls_output * F.one_hot(label.squeeze(), num_classes=self.num_classes)).sum(dim=1)
                uncond_output = torch.logsumexp(cls_output, dim=1)
                ...
                return cls_output, cond_output, uncond_output, cls_proxy, cls_embed
